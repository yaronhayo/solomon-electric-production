/**
 * IndexNow API Client (JavaScript version)
 ...
 * @param {string[]} urls - Array of URLs to submit (can be single URL or multiple)
 * @returns {Promise<Object>} Promise with submission results
 */
 ...
 * @param {string} sitemapPath - Path to sitemap file (XML)
 * @param {number} limit - Maximum number of URLs to submit (default: 10000, IndexNow max)
 */
export async function submitFromSitemap(sitemapPath, limit = 10000) {
  try {
 ...
 * @param {string} sitemapIndexPath - Path to sitemap index file
 * @param {number} limit - Maximum URLs per child sitemap
 */
export async function submitFromSitemapIndex(sitemapIndexPath, limit = 10000) {
  try {
